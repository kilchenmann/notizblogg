...
<?php
	...
	define ('BASE_FOLDER',	basename($root['dirname']));	// = admin
	$substringFolder = "-".mb_strlen(BASE_FOLDER);
	define ('SITE_ROOT',	substr($root['dirname'], 0, $substringFolder-1));	// = /var/www/notizblogg
	
	define ('SITE_URL',		'http://'.$_SERVER['HTTP_HOST'].'/'.$root['dirname']); // = http://chip.iml.unibas.ch/notizblogg


	//include ("checkuser.php");
	//include ("fn_show.php");
	//include ("fn_edit.php");
	include (__DIR__."/common/php/db.php");
//	include ("fn/content.php");
//	include ("fn/getNote.php");
//	include ("fn/getSource.php");
	//$sitePath = $_SERVER["REQUEST_URI"];
	//connect();
		$siteTitle = "Notizblogg | intern";
	//disconnect();
	$folder = "";
...
<?php
	...
	echo "<link rel='shortcut icon' href='__DIR__./common/images/favicon.ico'>";
?>
		<!-- <link type="text/css" media="screen" href="css/screen.css" rel="stylesheet" /> -->
	<style type="text/css">
		@import 'cssconst.php?c=screen.css';
	</style>
	
	<!--
	<link type="text/css" media="print, embossed" href="css/print.css" rel="stylesheet" />
	-->
	...
	<header>
		...
		<table class="settingBar" cellspacing="0" cellpadding="0">
			<tbody>
				<tr class="setting">
					<td>
						<button value="paper" class="changeView"><img src="../css/images/paper.png" alt="paper"></button>
					</td>
					<td>
						<a href="index.php" title="Back to the index (home)"><button value="home" class="home"><img src="../css/images/home.png" alt="home"></button></a>
					</td>
					<td>
						<button value="settings" class="menuSet"><img src="../css/images/settings.png" alt="settings"></button>
					</td>
				</tr>
			</tbody>
		</table>
...
?>
		<script type="text/javascript">
			...
			$(".titleIndex .right").html("#notes: 50");
		</script>
<?php
		echo "<div class='desk'>";
			$sql = mysql_query("SELECT noteID FROM note ORDER BY date DESC LIMIT 50");
